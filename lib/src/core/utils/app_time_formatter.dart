...
class AppTimeFormater {
  ...
  static TimeOfDay getTime(String time) {
    try {
      ...
      if (timeSplited.length != 2) throw const TimeException("Formato de tempo incorreto");
      if (int.parse(timeSplited[0]) > 23) throw const TimeException("Hora inválida");
      if (int.parse(timeSplited[1]) > 59) throw const TimeException("Minuto inválido");
